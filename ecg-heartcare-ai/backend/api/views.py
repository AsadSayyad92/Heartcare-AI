...
class PredictECGView(APIView):
    def post(self, request):
        ...
        backend = model_info.get("backend", "torch")
        ...
        if input_data is None or len(input_data) != expected_size:
            return Response(
                {"error": f"Input must be a list of {expected_size} floats."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if model_name not in loaded_models:
            ...
            elif backend == "adaboost":
                model = model_class(model_path)

            else:
                return Response({"error": f"Unknown backend '{backend}'"},
        ...
        elif backend == "xgboost" or backend == "adaboost":
            pred_class, probs = model.predict(input_data)

        # Fetch human label from DB
        try:
